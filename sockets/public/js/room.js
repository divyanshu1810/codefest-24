...
function startCall() {
    navigator.mediaDevices.getUserMedia(mediaConstraints)
        .then(localStream => {
            ...
            socket.emit('sendStream', { stream: localStream, room: roomid });
        })
...
sendButton.addEventListener('click', async() => {
    ...
    if(msg.charAt(0) == '/'){
        msg = msg.substring(1);
        const data = await fetch(`https://asia-southeast1-aiplatform.googleapis.com/v1/projects/authenticheck/locations/asia-southeast1/publishers/google/models/text-bison:predict`,{
            method: 'POST',
            headers: {
        ...
        console.log(res);
        messageField.value = '';
        socket.emit('message', JSON.stringify(res.text), username, roomid);
    }
    else{
...
document.addEventListener('DOMContentLoaded', function() {
    const rightCont = document.querySelector('.container-room .right-cont');
    const hamburgerMenu = document.querySelector('.container-room .hamburger-menu');
  
    if (hamburgerMenu) {
      hamburgerMenu.addEventListener('click', function() {
        rightCont.style.display = (rightCont.style.display === 'none' || rightCont.style.display === '') ? 'block' : 'none';
      });
    }
  });

document.querySelector('#vcont').addEventListener('click', function(){
    console.log('clicked');
    const rightCont = document.querySelector('.container-room .right-cont');
    rightCont.style.display = (rightCont.style.display === 'none' || rightCont.style.display === '') ? 'block' : 'none';
});